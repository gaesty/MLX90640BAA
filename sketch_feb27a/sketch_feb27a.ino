#include <WiFi.h>
#include <WiFiManager.h> // Ajout de la bibliothèque WiFiManager
#include <WebServer.h>
...
void setup() {
  ...
  // Décommenter la ligne suivante pour effacer les identifiants enregistrés (utile pour les tests)
  // wifiManager.resetSettings();

  Serial.println("Connexion au WiFi via WiFiManager...");
  
  // Tente de se connecter au réseau sauvegardé. Si échec, crée le point d'accès "CameraThermique_AP"
  if (!wifiManager.autoConnect("CameraThermique_AP")) {
    Serial.println("Échec de la connexion et expiration du délai");
    delay(3000);
    // Redémarre l'ESP pour réessayer
    ESP.restart();
  }
